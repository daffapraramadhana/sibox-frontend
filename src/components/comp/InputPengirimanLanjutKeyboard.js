...
class InputPengirimanLanjutKeyboard extends Component {
  constructor() {
    super();
    this.state = {
      layoutName: "key",
      inputName: "inputekspedisi",
      input: {
        inputekspedisi:
  ...
  render() {
    ...
    //   input.inputekspedisi = Cookies.get("pengirim").toString();
    ...
    // aman
    Cookies.set("ekspedisi", input.inputekspedisi);
      ...
      <div>
        <Form style={{ fontSize: "25px" }}>
          <Form.Group controlId="formPengirim">
            ...
            <Form.Control
              onFocus={() => {
                this.setActiveInput("inputekspedisi");
                this.setState({ layoutName: "key" });
              }}
              value={input["inputekspedisi"] || ""}
              onChange={(e) => this.onChangeInput(e)}
              type="text"
              autoComplete="off"
              placeholder="Masukan Nama Ekspedisi Paket"
              style={{ fontSize: "30px" }}
            />
          </Form.Group>
...
export default InputPengirimanLanjutKeyboard;
